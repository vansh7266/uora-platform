...
# Anything smaller than this byte threshold is treated as the legacy 1-page
# stub from the pre-WeasyPrint era and is regenerated on the fly.
_PDF_STUB_THRESHOLD = 5 * 1024  # 5 KB


async def _regenerate_report_pdf(submission_id: str, team: str, pdf_path: str) -> bool:
    """Rebuild a PDF report from data we still have in Postgres + Redis.

    Returns True on success. Used as a self-heal when the on-disk PDF is
    missing or is the legacy stub (< 5 KB) from before the WeasyPrint libs
    were installed in the benchmarker container.
    """
    ...
    try:
        async with timescale_pool.acquire() as conn:
            row = await conn.fetchrow(
                """
                SELECT throughput, correctness_rate,
                       p50_latency_ns, p90_latency_ns, p99_latency_ns,
                       success_rate, error_rate, max_tps,
                       resource_penalty, composite_score, anomaly_score,
                       language, team
        ...
        score_payload = {
            "submission_id":   submission_id,
            "team":            row["team"] or team,
            "language":        row["language"] or "",
            "composite_score": float(row["composite_score"] or 0),
            "throughput": {
                "avg": float(row["throughput"] or 0),
                "max": float(row["max_tps"] or row["throughput"] or 0),
                "unit": "orders/sec",
            },
            "latency": {
                "p50_ms": float((row["p50_latency_ns"] or 0) / 1_000_000),
                "p90_ms": float((row["p90_latency_ns"] or 0) / 1_000_000),
                "p99_ms": float((row["p99_latency_ns"] or 0) / 1_000_000),
            },
            "correctness": {
                "rate":       float(row["correctness_rate"] or 0),
                "percentage": f"{float(row['correctness_rate'] or 0) * 100:.2f}%",
            },
            "reliability": {
            ...
            "anomaly": {
                "score":       float(row["anomaly_score"] or 0),
                "is_anomaly":  float(row["anomaly_score"] or 0) >= 0.5,
                "confidence":  0.0,
                "reason":      "",
            },
            "resource_penalty": float(row["resource_penalty"] or 0),
            "formula": "(throughput x correctness_rate x success_rate) / (p99_latency_ms + resource_penalty^2)",
        }
        # Run the generator in a thread — it does sync I/O + (optionally)
        # WeasyPrint which is blocking.
        import asyncio
        from uora.scoring.report_generator import ReportGenerator
        def _do() -> None:
            rg = ReportGenerator()
            html = rg.generate_html(submission_id, score_payload, [],
                                    {"is_anomaly": score_payload["anomaly"]["is_anomaly"],
            ...
            rg.generate_pdf(
                ...
                violations=[],
                anomaly={
                    "score":      score_payload["anomaly"]["score"],
                    "is_anomaly": score_payload["anomaly"]["is_anomaly"],
                    "reason":     "Within training manifold." if not score_payload["anomaly"]["is_anomaly"] else "Outside training manifold.",
                },
            )
        await asyncio.get_event_loop().run_in_executor(None, _do)
        return os.path.exists(pdf_path) and os.path.getsize(pdf_path) >= _PDF_STUB_THRESHOLD
    except Exception as exc:
        logger.error("Report regeneration failed for %s: %s", submission_id, exc)
        return False


@app.get("/api/v1/report/{submission_id}")
async def download_report(submission_id: str, auth_data: dict = Depends(require_auth)):
    """Download the PDF performance report for a scored submission.

    Reports are normally written by the scoring engine at
    ``/tmp/uora-reports/report-{submission_id}.pdf``. If that file is missing
    or is the legacy stub (< 5 KB, written before WeasyPrint was installed),
    we transparently rebuild it from the score row in Postgres so the user
    never sees a broken or one-page PDF.
    """
    if not redis_pool:
        raise HTTPException(503, "Service temporarily unavailable")
    ...
    if data.get("status") != "scored":
        raise HTTPException(409, f"Report not ready (status: {data.get('status', 'unknown')})")

    report_dir = os.getenv("REPORT_OUTPUT_DIR", "/tmp/uora-reports")
    pdf_path = os.path.join(report_dir, f"report-{submission_id}.pdf")

    needs_regen = (
        not os.path.exists(pdf_path)
        or os.path.getsize(pdf_path) < _PDF_STUB_THRESHOLD
    )
    if needs_regen:
        team_for_regen = data.get("team", "engine") or "engine"
        ok = await _regenerate_report_pdf(submission_id, team_for_regen, pdf_path)
        if not ok:
            raise HTTPException(404, "Report file not found and could not be regenerated")

    team = data.get("team", "engine") or "engine"
    filename = f"uora-report-{team.lower().replace(' ', '_')}-{submission_id[:8]}.pdf"
    return FileResponse(
        pdf_path,
