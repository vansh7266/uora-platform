...
async def download_report(submission_id: str, auth_data: dict = Depends(require_auth)):
    ...
    Reports are generated by the scoring engine after benchmark completion
    and written to /tmp/uora-reports/report-{submission_id}.pdf (path is
    configurable via REPORT_OUTPUT_DIR).
    """
    if not redis_pool:
        raise HTTPException(503, "Service temporarily unavailable")

    # Verify submission belongs to the caller (or is public-readable).
    data = await redis_pool.hgetall(f"submission:{submission_id}")
    if not data:
        raise HTTPException(404, "Submission not found")
